...
// Place all your Javascript code inside this "document ready" function so it does not run until the DOM is ready for Javascript manipulation.
    ...
    //  Create a function that listens for clicks on the voting buttons	
	...
	//  When a button is clicked, look at the `data-vote` attribute to determine what the user is voting for ("great" or "greatest").	
		if (clickVote == 'great'){					//  Increment the counter for whichever vote talley is affected.
			voteCounts.great++;
			voteCounts.total++;
		}
